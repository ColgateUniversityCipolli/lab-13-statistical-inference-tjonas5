...
**Solution**
In order to collect the data required for Figure 2(g), I copied the data from the sheets g.Farther_vals and g.Closer_vals and pasted it into a new sheet, in columns with headings. Then I read the data using read_csv. As we discussed in lab, I created the column that has the differences between the closer and farther data in excel rather than using mutate. 

```{r}
data.birds = read_csv("BirdFartherCloserData.csv")
```
...
**Solution**
In this code, I first calculate the mean and standard deviation for each column in my data: Farther, Closer, and Difference. Then I chose to make a boxplot to represent the data for each column. As we can see in the boxplots, the data suggests that the dopamine in the brains of birds decreases when they sing futher away. It also suggests that 

```{r}
(farthermean = mean(data.birds$Farther))
(farthersd = sd(data.birds$Farther))
(closermean = mean(data.birds$Closer))
(closersd = sd(data.birds$Closer))
(diffmean = mean(data.birds$Difference))
(diffsd = sd(data.birds$Difference))
```

```{r}
#| label: fig-box-farther
#| fig-cap: "Corrected dopamine from the worst syllable quality"
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
#| size: scriptsize
ggplot(data = data.birds, aes(x=Farther)) +
  geom_boxplot() +
  coord_flip() +
  theme_bw() + 
  labs(x = "Dopamine (corrected)",
        y = NULL)
```

```{r}
#| label: fig-box-closer
#| fig-cap: "Corrected dopamine from the best syllable quality"
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
#| size: scriptsize
ggplot(data = data.birds, aes(x=Closer)) +
  geom_boxplot() +
  coord_flip() +
  theme_bw() + 
  labs(x = "Dopamine (corrected)",
...
```{r}
#| label: fig-box-diff
#| fig-cap: "Difference in corrected dopamine between both syllable qualities"
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
#| size: scriptsize
ggplot(data = data.birds, aes(x=Difference)) +
  geom_boxplot() +
  coord_flip() +
  theme_bw() + 
  labs(x = "Dopamine (corrected)",
...
For each column of data (Farther, Closer, and Difference), I performed a t test. The null hypothesis was equal to zero. 
```{r}
mu0 = 0 
t.test(data.birds$Farther, alternative = "less", mu = mu0)
t.test(data.birds$Farther)
t_farther = -7.92
```

The far responses differed significantly from 0 ($t = -7.92$, $p = 1.86 * 10^(-8)$, CI: -$.268$, $-.157$). This means that the null hypothesis can be rejected, and that the observed data is nearly 8 standard deviations away from the mean if the null was true. 

```{r}
t.test(data.birds$Closer, alternative = "greater", mu = mu0)
t.test(data.birds$Closer)
t_closer = 9.15
```

The close responses differed significantly from 0 ($t = 9.15$, $p = 1.33 * 10^(-9)$, CI: $.138$, $.218$). This means that the null hypothesis can be rejected, and that the observed data is more than 9 standard deviations away from the mean if the null was true. 

```{r}
t.test(data.birds$Difference, alternative = "two.sided", mu = mu0)
t_difference = 9.14
```

The difference between populations was significant ($t = 9.14$, $p = 2.75 * 10^(-9)$, CI: $.303$, $.479$). This means that the null hypothesis can be rejected, and that the observed data is more than 9 standard deviations away from the mean if the null was true. 
...
### Birds are farther
This code is for the first hypothesis test, when the birds are farther. 
...
s <- sd(data.birds$Farther)
t.breaks <- seq(-24, 24, 4)
xbar.breaks <- round(t.breaks * (s/sqrt(n)) + mu0, 2)
ts <- rep(NA, 1000)
n <- nrow(data.birds)
for(i in 1:1000){
  curr.sample <- sample(data.birds$Farther, size=n, replace=T)
...
```{r}
#| label: fig-density-further
#| fig-cap: "Null T distribution and the observed T statistic (solid red)"
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
ggplot(ggdat) +
 ...
 geom_ribbon(data = ggdat |> filter(t < tval_05),
  aes(x = t, ymin=0, ymax=pdf.h0),
  fill="darkorchid2", alpha=0.50)+
 geom_hline(yintercept=0) +
 # Plot observation
 geom_point(data=tibble(x=t_farther, y=0),
...
Note that the resampling distribution is in light grey. This distribution, which is the sampling distribution estimated using only the data, does not appear to overlap at all with the null hypothesis distribution that is centered at zero. 
...
s <- sd(data.birds$Closer)
xbar.breaks <- round(t.breaks * (s/sqrt(n)) + mu0, 2)
ts <- rep(NA, 1000)
for(i in 1:1000){
  curr.sample <- sample(data.birds$Closer, size=n, replace=T)
  ts[i] <- (mean(curr.sample) - mu0)/(s/sqrt(n))
}
tval_05 = -qt(.05, 24)
```
...
ggplot(ggdat) +
 ...
 geom_hline(yintercept=0) +
 # Plot observation
 geom_point(data=tibble(x=t_closer, y=0),
 aes(x=x, y=y), color="red", size=3) +
 theme_bw()+
 # Specify axis ticks
 scale_x_continuous("t", breaks=t.breaks,
...
### Difference between closer and farther 

```{r}
ggdat = tibble(t = seq(-4, 4, length.out=1000)) |>
  mutate(pdf.h0 = dt(t, df =n-1))

s <- sd(data.birds$Difference)
xbar.breaks <- round(t.breaks * (s/sqrt(n)) + mu0, 2)
ts <- rep(NA, 1000)
for(i in 1:1000){
  curr.sample <- sample(data.birds$Difference, size=n, replace=T)
  ts[i] <- (mean(curr.sample) - mu0)/(s/sqrt(n))
}
...
ggplot(ggdat) +
 geom_line(aes(x=t, y=pdf.h0))+ # plot distribution of T under H0
 stat_density(data=tibble(t=ts), aes(x=t), color="grey", geom="line")+
 geom_ribbon(data = ggdat |> filter(t > tval_025),
  aes(x = t, ymin=0, ymax=pdf.h0),
  fill="darkorchid2", alpha=0.50)+
 geom_hline(yintercept=0) +
 # Plot observation
 geom_point(data=tibble(x=t_difference, y=0),
 aes(x=x, y=y), color="red", size=3) +
 theme_bw()+
 # Specify axis ticks
 scale_x_continuous("t", breaks=t.breaks,
