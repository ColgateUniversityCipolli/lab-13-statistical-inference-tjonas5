...
```{r}
df = read_csv("BirdFartherCloserData.csv")
```
...
```{r}
(farthermean = mean(df$Farther))
(farthersd = sd(df$Farther))
(closermean = mean(df$Closer))
(closersd = sd(df$Closer))
(diffmean = mean(df$Difference))
(diffsd = sd(df$Difference))
```
...
#| fig-align: "center"
#| size: scriptsize
ggplot(data = df, aes(x=Difference)) +
  geom_boxplot() +
  coord_flip() +
  theme_bw() + 
  labs(x = "Dopamine (corrected)",
...
```{r}
mu0 = 0 
t.test(df$Farther, alternative = "less", mu = mu0)
```

The far responses differed significantly from 0 ($t = -7.92$, $p = 1.86 * 10^(-8)$, CI: -$\inf$, $-.166$)

(closert.stat = (closermean - mu0)/(closersd/sqrt(n)))

(difft.stat = (diffmean - mu0)/(diffsd/sqrt(n)))
```
