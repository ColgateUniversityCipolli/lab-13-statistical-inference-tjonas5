...
**Solution**
I chose to make a boxplot to represent the 
...
For each column of data (Farther, Closer, and Difference) I calculated the mean and standard deviation, and the corresponding t statistic to evaluate if the difference was significant or not. The null hypothesis was equal to zero, 
```{r}
farthersd = sd(data$Farther)
mu0 = 0 
n = 6
(farthert.stat = (farthermean - mu0)/(farthersd/sqrt(n)))
t.test(farthert.stat)


closersd = sd(data$Closer)
(closert.stat = (closermean - mu0)/(closersd/sqrt(n)))



diffsd = sd(data$Difference)
